...
Browser-based broadcast test-signal generator for [videotooling.com](https://videotooling.com). Renders a suite of known-good signals — color bars (HD SMPTE, SMPTE EG 1, PAL 75/100%), calibration and diagnostic patterns (PLUGE black-level, a moving pattern, per-frame flicker, a field-order sweep), and an A/V lip-sync pattern — alongside a 1 kHz reference tone, running timecode, and optional label / safe-area burn-ins. Everything is generated **client-side with ffmpeg.wasm** (no server, no uploads) and exported as H.264/AAC in MP4, MOV, or MPEG-TS. Useful for validating ingest paths, encoder configs, and sync/color checks without spinning up OBS or a hardware pattern generator.

## Features

- **Patterns:** HD SMPTE bars (RP 219), classic SMPTE bars (EG 1), PAL/EBU 75% and 100% bars, `testsrc` moving pattern (catches frozen frames), lip-sync (white flash + 100 ms beep every second, frame-aligned), PLUGE (−2/0/+2/+4% near-black steps + 75% white reference, exact code values 11/16/21/26), field sweep (fast-moving bar, geometry scaled to the frame — wrong field order shows as judder), black/white per-frame flicker (dropped/repeated frames show instantly; becomes field-rate alternation in 1080i)
- **Formats:** resolution × frame-rate matrix — 3840×2160, 2560×1440, 1920×1080, 1280×720, 960×540, 640×360 at 23.98/24/25/29.97/30/50/59.94/60p, plus 50i and 59.94i at 1080 (TFF, interlaced coding flags). Large frames get a proportionally shorter duration cap to fit the wasm memory budget (75 s at 2160)
- **Burn-ins:** running timecode (SMPTE drop-frame for the 29.97/59.94 family, non-drop otherwise), optional custom label text, safe-area markers (93% action / 90% title + center cross, SMPTE ST 2046-1)
